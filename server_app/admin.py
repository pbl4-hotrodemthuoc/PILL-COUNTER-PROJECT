...
def dashboard():
    ...
    total_prescriptions = DonThuoc.query.count()
    # pending_incidents_count đã có sẵn từ context_processor, nhưng chúng ta có thể tính lại ở đây cho rõ ràng
    pending_incidents = BaoCaoSuCo.query.filter_by(trang_thai=TrangThaiSuCoEnum.pending).count()
    online_devices = ThietBi.query.filter_by(trang_thai=TrangThaiThietBiEnum.online).count()
    total_devices = ThietBi.query.count()

    # Truyền các biến thống kê vào template
...
@admin_required
def system_logs():
    """Hiển thị nhật ký hệ thống với các bộ lọc."""
    ...
    # Lấy danh sách người dùng để hiển thị trong bộ lọc
    all_users = NguoiDung.query.order_by(NguoiDung.ho_ten).all()

    return render_template('admin/system_logs.html', 
                           logs=logs,
                           all_users=all_users,
                           LoaiLogEnum=LoaiLogEnum,
                           filters={'log_type': log_type, 'user_id': user_id, 'log_date': log_date_str})
